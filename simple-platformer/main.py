...
def create_level():
	global blockList, tokenList
	blockList = []; 
	tokenList = []
	for y in range(0, len(level1)):
		for x in range(0, len(level1[y])):

			if level1[y][x] == 1: ## stationary blocks
...
def game_loop():
	...
	player = Player(START_X, START_Y, BLACK, PLAYER_WIDTH, PLAYER_HEIGHT)            ## Create player object
	...
	while gameLoop:
		...
		window.blit(player.image, camera.apply(player))    ## player
